subroutine output_parallel(v, Lx, Ly)
  ...
  integer(MPI_ADDRESS_KIND) :: lb, extent_real8, sizeof_v, target_disp, a0, a1
  real(8)             :: time
  ...
  call mpi_win_create(v, sizeof_v, 1, MPI_INFO_NULL, MPI_COMM_WORLD, win, ierr)
  ...
  if (decomp%my_rank == 0) then
     do y = 0, decomp%Ny + 1
        do x = 0, decomp%Nx + 1
           ...
           call mpi_get_address(v(0, 0), a0, ierr)
           call mpi_get_address(v(x_local, y_local), a1, ierr)
